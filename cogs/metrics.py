...
class Metrics(commands.Cog):
	...
	async def plot(self,ctx):
		...
		data_frame.plot(title=f"{ctx.guild.name} roles on {datetime.today().strftime('%Y-%m-%d')}",kind='bar', width = .2,rot=90,fontsize=12,legend=False,figsize=(20+(sizing)*2,10+(sizing)))
